...
def main(argv):
    ...
    ppt_version_path = os.path.join(args.current_project_directory, ".ppt-version")
    if not os.path.exists(ppt_version_path):
        raise SystemExit("PPT version cookie not found: {0}".format(ppt_version_path))
    ...
    if not os.path.exists(metadata_path):
        raise SystemExit("Project metadata file not found: {0}".format(metadata_path))

    # Setup the new PPT directory.
    print("Setting up new PPT directory...")
    new_ppt_directory = mkdtemp(prefix="ppt-new-")
    test.main(["unused_progname", "--metadata-path", metadata_path, new_ppt_directory])
    ...
    for git_path in git_ls_files:
        ...
        for dir_ in dirs:
            ...
            if not os.path.exists(path):
                path = "/dev/null"
            paths.append(path)
        process = subprocess.Popen(["diff3", "--merge"] + paths, stdout=subprocess.PIPE)
        diff3_out, diff3_err = process.communicate()
        if process.returncode != 0:
            print(
                "{0}: merge conflicts found, please resolve manually".format(paths[0])
            )
        with open(paths[0], "w") as current_file:
            current_file.write(diff3_out)

    new_revision = subprocess.check_output(["git", "rev-parse", "HEAD"]).rstrip()
